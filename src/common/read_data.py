...
class ReadData:
	...
	def table_is_norm(self) -> bool:
		norm_list = ["环境", "模块名", "接口名称", "请求方法", "请求头", "请求参数", "用例名称", "请求体", "预期结果"]
	...
	def get_module_data(self, fieldName=None) -> list:
		...
		for num in range(1, self.get_sheetData().nrows):
			if self.get_sheetData().row_values(num)[fieldName_num] == fieldName:
				Table_data.append(self.get_sheetData().row_values(num))
		return Table_data
